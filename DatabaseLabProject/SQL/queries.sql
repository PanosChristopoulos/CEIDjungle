...
COUNT(has_degree.degr_title) AS 'degree number', AVG(has_degree.grade) AS 'average degree grade' 
FROM candidate 
...
SELECT candidate.username, COUNT(applies.cand_usrname) AS 'application number', AVG(job.salary) AS 'average salary' 
FROM candidate 
...
SELECT recruiter.username, COUNT(DISTINCT interviews.interview_id) as Interviews, 
COUNT(DISTINCT job.id) as Jobs, AVG(job.salary) as AvgSalary 
FROM recruiter 
INNER JOIN job ON recruiter.username = job.recruiter 
INNER JOIN interviews ON recruiter.username = interviews.recr_name  
