...
recr_name VARCHAR(12) NOT NULL,
cand_name VARCHAR(12) NOT NULL,
interview_start DATETIME DEFAULT CURRENT_TIMESTAMP,
interview_duration_minutes INT NOT NULL,
interview_job INT NOT NULL,
comments TEXT NULL,
personality_score TINYINT DEFAULT 0,
education_score TINYINT DEFAULT 0,
experience_score TINYINT DEFAULT 0,
PRIMARY KEY (interview_id),
CONSTRAINT RECR_NAME FOREIGN KEY (recr_name) REFERENCES `recruiter`(username) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT CANDIDATE_NAME FOREIGN KEY (cand_name) REFERENCES `candidate`(username) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT INTERVIEWTARGETJOB FOREIGN KEY(interview_job) REFERENCES `job`(id) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE Sections(
SecTitle VARCHAR(40) NOT NULL,
info TEXT NULL,
parentSection VARCHAR(20) DEFAULT NULL,

PRIMARY KEY(SecTitle),
CONSTRAINT SECTIONHIERARCHY FOREIGN KEY (parentSection) REFERENCES `Sections`(SecTitle) ON DELETE CASCADE ON UPDATE CASCADE
);

INSERT INTO Sections VALUES
("Electrical Engineering", "Extensive design and application of electronics", NULL),
("Software Engineering", "A collection of data or computer instructions that tell the computer how to work", NULL),
("System Software ", "It provides a platform for running application software", "Software Engineering"),
("Operating systems ", "Essential collections of software that manage resources", "System Software"),
("Device drivers ", "Each device needs at least one corresponding device driver", "System Software"),
("Application software", "Software designed to perform a group of coordinated functions for user's benefit", "Software Engineering"),
("HTML", "Standard markup language for documents designed to be displayed in a web browser", "Application Software"),
("Object Oriented Programming",  "Programming paradigm based on the concept of objects", "Application Software"),
("Computer architecture", "Set of rules that describe the functionality and implementation of computer systems", "Electrical Engineering"),
("Microarchitecture", "Description of how a particular processor implements the ISA", "Computer Architecture"),
("System Design", "Data processing other than the CPU, such as direct memory access (DMA)", "Computer Architecture"),
("Artificial Intelligence", "Intelligence demonstrated by machines", "Electrical Engineering"),
("Machine Learning", "Algorithms that computer systems use in order to perform a specific task effectively", "Artificial Intelligence"),
("Robotics", "design, construction, operation, and use of robots", "Artificial Intelligence");

CREATE TABLE CompanySections(
companyAFM char(9) NOT NULL,
CSection VARCHAR(40) NOT NULL,
CONSTRAINT COMPANYSECTIONAFM FOREIGN KEY (companyAFM) REFERENCES `etaireia`(AFM) ON DELETE CASCADE ON UPDATE CASCADE,
CONSTRAINT COMPANYSECTIONNAME FOREIGN KEY (CSection) REFERENCES `sections`(SecTitle) ON DELETE CASCADE ON UPDATE CASCADE);

INSERT INTO CompanySections VALUES
('023451232',"System Design"),
('123432211', "Robotics");
...
LogUser ENUM('candidate', 'recruiter', 'job', 'etaireia'),
PRIMARY KEY(logID),
CONSTRAINT  LOGUSER FOREIGN KEY (username) REFERENCES `user`(username) ON DELETE CASCADE ON UPDATE CASCADE
);

INSERT INTO interviews(recr_name, cand_name,interview_job, interview_duration_minutes,comments, personality_score, education_score, experience_score) VALUES
("bettyg", "zazahir23",1, 40, "Overall adequately educated and experienced individual", 2, 4, 4),
("Giankost", "liagourma", 2, 45, "Positive and educated, but with minor experience", 4, 4, 1),
("varcon", "mnikol", 1, 50, "Long Experience in required field", 3,4,5),
("pavkie", "lionarF", 4, 25, "Well Educated but arrogant personality", 1,4,3),
("msmith", "liagourma", 3, 40, "Young and aspiring prospect for our company", 5,4,2);
